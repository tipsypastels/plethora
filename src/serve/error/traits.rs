use super::{Render, ServeError, ServeResult};
use anyhow::Error;
use std::future::Future;

pub trait Re<T> {
    fn re(self, re: &impl Render) -> ServeResult<T>;
}
...
pub trait ReFuture<T> {
    async fn re(self, re: &impl Render) -> ServeResult<T>;
}
...
pub trait OrNotFound<T> {
    fn or_not_found(self, re: &impl Render) -> ServeResult<T>;
}
